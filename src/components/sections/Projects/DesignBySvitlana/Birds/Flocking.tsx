...
interface Uniforms {
  [key: string]:
    | THREE.IUniform<THREE.Texture | number | THREE.Vector3>
    | undefined;

  time: THREE.IUniform<number>;
  delta?: THREE.IUniform<number>;
  texture: THREE.IUniform<THREE.Texture>;
  separationDistance?: THREE.IUniform<number>;
  alignmentDistance?: THREE.IUniform<number>;
  cohesionDistance?: THREE.IUniform<number>;
  freedomFactor?: THREE.IUniform<number>;
  predator?: THREE.IUniform<THREE.Vector3>;
}
...
const Flocking = () => {
  ...
  const positionUniforms = useMemo<Uniforms>(
    () => ({
      time: { value: 1.0 },
  ...
  const velocityUniforms = useMemo<Uniforms>(
    () => ({
      time: { value: 1.0 },
      texture: { value: new THREE.Texture() },
    }),
  ...
  useEffect(() => {
    ...
    // Shader code (Fragment and Vertex)
...
`;
  ...
  }, [birdUniforms, last, positionUniforms, velocityUniforms]);
